...
Annealing.prototype = {
	...
	stepsPerT: null,
	step: 0,
	init: functio (opts, width, height, listener){
		this.points = opts.points;
		this.card = this.points.length;
		this.t0 = opts.t0;
		this.g = opts.g;
		this.stepsPerT = opts.stepsPerT;
		this.stopValue = 0.000001;
		this.listener = listener;
		this.width = width;
		this.height = height;
		for (var i = 0; i < this.card; i += 1){
	...
	cycle: function (){
		...
		if (!this.currentWeight){
			...
			newMin = this.addptPath(tmpPath, w);
		} else {
	...
	computeWeight: function (path){
		...
		for (var i = 0; i < this.card; i += 1){
			...
			var y1 = this.points[idx].x;

			weight += Math.sqrt((x1 - x0) * (x1 - x0) + (y1 - y0) * (y1 - y0));
		}
	...
	oneStep: function (){
		...
		var v1 = this.currentPath.slice(j, this.card);

		return v1.concat(v2.reverse().oncat(v3));
	}
};
